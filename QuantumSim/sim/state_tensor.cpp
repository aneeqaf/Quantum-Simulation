...
TensorProductStateVector::
    TensorProductStateVector(const int qubits,
                             ...
                             const int verb) : qp(cut_type == QubitPartition::Cuts::Horizontal ? QubitPartition(cut_type, qubits, row_major, hcut, first_part_small)
    ...
    CountXCZGates(const bitset<128> *__restrict CZ_bitmasks)
{
    ...
    for (int i = 0; i < num_q_a; ++i)
    {
        while (xCZ_bitmask[i] != 0)
        {
            ...
            const int q1 = first_half ? __builtin_ctzl(first_half) : second_half ? 64 + __builtin_ctzl(second_half)
                                                                                 : 0;
            xCZ_bitmask[i][q1] = 0;
        }
    }
    ...
    GetGlobalFactorPower() const
{
    if (partition_to_sim == 'a')
        ...
        return state_a->GetGlobalFactorPower() > state_b->GetGlobalFactorPower() ? state_a->GetGlobalFactorPower() : state_b->GetGlobalFactorPower();
}
    ...
    for (idx_size a = 0; a < num_ranges_a; ++a)
    {
        ...
        for (idx_size b = 0; b < num_ranges_b; ++b)
        {
            ...
            if ((real(s_b) > 1e-20 || imag(s_b) > 1e-20) &&
                ...
                entropy += norm(s_a * s_b * rescaling_factor_a * rescaling_factor_b) * log2l(norm(s_a * s_b * rescaling_factor_a * rescaling_factor_b));
        }
    }
    ...
    CountZeroAmpPercentage() const
{
    ...
    if (calls % 2)
        ...
        return state_b->CountZeroAmpPercentage();
}
