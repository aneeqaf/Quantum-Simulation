    ...
    ApplyXCZGatesExact(const bitset<128> *__restrict CZ_bitmasks)
{
    ...
    if (book_keep)
    {
        ...
        //        }
        count_of_category.xCZ_not_applied += tensor_addends[0]->CountXCZGates(CZ_bitmasks) - (count_of_category.decomposed_CZ - prev_CZ_count);
    }
                              ...
                              int th)
{
    ...
    if (book_keep)
    {
        if (count_of_category.X1_2 - prev_X_count)
            count_of_category.X1_2 -= (num_addends - 1) * (count_of_category.X1_2 - prev_X_count) / (num_addends);
        if (count_of_category.Y1_2 - prev_Y_count)
            count_of_category.Y1_2 -= (num_addends - 1) * (count_of_category.Y1_2 - prev_Y_count) / (num_addends);
    }
}
