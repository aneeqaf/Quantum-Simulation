...
void GroupCZGates(bitset<128> *__restrict qubits_CZ_bitmasks,
...
void GroupTGates(bitset<128> *__restrict T_bitmasks,
                 const int num_qubits_amp,
                 const vector<idx_size> &gate_qubits)
{
                          ...
                          const idx_size *__restrict T_bitmasks)
{
    ...
    for (idx_size count = 0; count < amp_size; ++count)
    {
        ...
        const idx_size gate_c = __builtin_popcountll(gc & T_bitmasks[0]) + __builtin_popcountll(gc & T_bitmasks[1]);
                    ...
                    const int num_threads)
{
    ...
    //
    //    return i_count;
    const bool H_bitmask_applicable = (H_bitmask & (X_bitmask | Y_bitmask)) == (X_bitmask | Y_bitmask) && ((X_bitmask | Y_bitmask) != 0);
