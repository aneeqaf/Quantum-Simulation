            ...
            const idx_size *indices /*4*/)
{
    ...
    if (amp[indices[0]] == cmplx(0, 0) && amp[indices[1]] == cmplx(0, 0) && amp[indices[2]] == cmplx(0, 0) && amp[indices[3]] == cmplx(0, 0))
        return;
...
void Apply2MergedGatesHelper(cmplx *__restrict amp,
                             const idx_size gate_qubits,
                             const int num_qubits_amp,
                             const function &gate_func,
                             const idx_size data_parallelism = 1);

void ApplyHighQ2MergedGatesInParallel(cmplx *__restrict amp,
                   ...
                   const bool parallel = false)
{
    ...
    int num_gates_collected = (__builtin_ctzl(gates_bitmask) < num_qubits - 1 && __builtin_ctzl(gates_bitmask ^ (1ull << __builtin_ctzl(gates_bitmask))) < num_qubits - 2) ? 4 : 1;
...
void GroupCZGates(idx_size *__restrict qubits_CZ_bitmasks,
                  const int num_qubits_amp,
                  const vector<idx_size> &gate_qubits);

void GroupTGates(idx_size *__restrict T_bitmasks,
                 const int num_qubits_amp,
                 const vector<idx_size> &gate_qubits);

void ExtractIndicesForAmp(idx_size *strides,
...
void FormBlockOfXYHGates(vector<Gate> &cluster,
                         idx_size &gate_i,
                         const Gate::Type gate_type,
                         const vector<Gate> &all_gates);
void ApplyBlockOfCZTGates(cmplx *__restrict amp,
                          const int num_qubits_amp,
                          const idx_size *__restrict CZ_bitmasks,
                          const idx_size *__restrict T_bitmasks);

void ApplyCZDecomposition(cmplx *__restrict amp,
                          const int num_qubits_amp,
                          const int gate_qubit,
                          const Gate::Type gate_type);

void Apply1QXYHGates(cmplx *__restrict amp,
                     const idx_size num_threads,
                     const idx_size q,
                     const idx_size num_qubits,
                     const Gate::Type gate_type);

void Apply2MergedXY12Gates(Gate gate1,
                           Gate gate2,
