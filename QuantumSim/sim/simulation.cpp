                      ...
                      const GenericQuantumState &copy_amp)
{
    ...
    Time time;
    time.StartTime();
    static const idx_size cz_paths_ex = config->proc_prefix_bits ? 1ull << config->ranges_bits : 1ull << config->norm_depth;
    int total_cz_bits = config->proc_prefix_bits ? config->ranges_bits : config->norm_depth;
                           ...
                           Circuit &circuit)
{
    ...
    int total_cz_bits = config->proc_prefix_bits ? config->proc_prefix_bits + config->ranges_bits : config->norm_depth;
             ...
             Circuit &circuit)
{
    ...
    if (config->sim_type != Config::SimType::FullState)
    {
        ...
        {
            cerr << "\n\nxCZ path of length " + to_string(xCZ_path_len) + " does not equal " + to_string(twoq_gate_count.second) + " xCZ gates.\n";
            throw;
        }
    }
                     ...
                     const idx_size cz_path_len)
{
    ...
    else if (exec == 1)
    {
        ...
        {
            cout << "Simulated " + to_string(curr_gate) + " gates, including " + to_string(amp.count_of_category.decomposed_CZ / adjustment_factor) + " xCZ gates. ";
        ...
        {
            cout << "Simulated " + to_string(curr_gate) + " gates, including " + to_string(amp.count_of_category.decomposed_CZ / adjustment_factor) + " xCZ gates. No xCZ gates left.\n";
            if (config->dfs_length != 0)
        ...
        if (current_gate.IsDiagonal())
        {
            ...
            else
            {
                amp.ApplyCGate(current_gate.GetNumControls(), current_gate.GetQubits(),
                               current_gate, (Gate::Type)current_gate.GetType());
            }
        }
        else
        {
    ...
    PrintIdxsToFile() const
{
    ...
    string dir = "output/amp_vectors/" + config->infile + "_" + to_string(config->depth) + "_" + to_string(config->proc_prefix_bits + config->ranges_bits) + "_" + to_string(config->num_threads);
    if (config->approx)
    ...
    WriteMmapToASCIIFile() const
{
    ...
    for (idx_size i = 0; i < idx_print.size(); ++i)
    {
        ...
        if (imag(*(*config->mmap_obj)[i]) < 0)
            amp_out << imag(*(*config->mmap_obj)[i]) << "j";
        else
            amp_out << "+" << imag(*(*config->mmap_obj)[i]) << "j";
        amp_out << "\n";
    }
    ...
    WriteAmpToASCIIFile(GenericQuantumState &amp) const
{
    ...
    for (idx_size i = 0; i < idx_print.size(); ++i)
    {
        ...
        if (imag(amp[idx_print[i]]) < 0)
            amp_out << imag(amp[idx_print[i]]) << "j";
        else
            amp_out << "+" << imag(amp[idx_print[i]]) << "j";
        amp_out << "\n";
    }
                   ...
                   const Circuit &circuit) const
{
    ...
    if (config->proc_prefix_bits == 0)
    {
        ...
        if (benchmark.count(key0) || benchmark.count(key1))
        {
            ...
            if (real(amp[3]) - real(benchmark[key][0]) < 1e-9 && imag(amp[3]) - imag(benchmark[key][0]) < 1e-9 && real(amp[temp_amp_size / 4]) - real(benchmark[key][1]) < 1e-9 && imag(amp[temp_amp_size / 4]) - imag(benchmark[key][1]) < 1e-9 && real(amp[temp_amp_size / 2]) - real(benchmark[key][2]) < 1e-9 && imag(amp[temp_amp_size / 2]) - imag(benchmark[key][2]) < 1e-9 && real(amp[3 * temp_amp_size / 4]) - real(benchmark[key][3]) < 1e-9 && imag(amp[3 * temp_amp_size / 4]) - imag(benchmark[key][3]) < 1e-9 && real(amp[temp_amp_size - 3]) - real(benchmark[key][4]) < 1e-9 && imag(amp[temp_amp_size - 3]) - imag(benchmark[key][4]) < 1e-9)
            {
                cout << "passed\n";
            }
            else
...

    {
        ...
        {
            string CZ_T_s = "\tCZ & T (" +
                            to_string((amp.count_of_category.CZ_T - amp.count_of_category.decomposed_CZ) / adjustment_factor) + "), Low X & Y (" + to_string(amp.count_of_category.low_q_XY1_2) + ")";
        ...
        {
            string XY_s = "\tSingle X (" + to_string(amp.count_of_category.X1_2) + ") & Y (" + to_string(amp.count_of_category.Y1_2) + ")";
        ...
        {
            string X_Y_s = "\tMerged X & Y (" + to_string(amp.count_of_category.merged_XY1_2) + ")";
