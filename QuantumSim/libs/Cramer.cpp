...
num_turnings(num_turnings), lambda(0), k(0), projection_vector(projection_v), dist_type(exponential)
{
    ...
    */
    num_turnings = NUM_TURNINGS_CW[num_bits_codewords];
    ...
    codewords_spacing = ((double)total_spiral_length/(double)(1ull << num_bits_codewords));
...
{
    return _mm256_div_ps(_mm256_sub_ps(CalcApproxSpiralLenAVX(thetas), _mm256_set1_ps(spiral_length_r)),
                         _mm256_set1_ps(codewords_spacing));
}
