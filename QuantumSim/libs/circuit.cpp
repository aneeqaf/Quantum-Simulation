    ...
    GroupSimilarGates()
{
    ...
    for (idx_size j = qubits; j < gates.size() && (g_i + last_Y + last_X + last_T + last_CZ) < gates.size(); ++j)
    {
        ...
        if (IsCrossingGate(i))
        {
            ...
            for (; j < gates.size(); ++j)
            {
                ...
                if (IsCrossingGate(j))
                    ...
                    if (num_xCZ_in_cluster == num_crossing_q)
                        break;
            }
            ...
            if (remaining_path_bits <= num_xCZ_in_cluster)
            {
                ...
                for (; j < gates.size(); ++j)
                {
                    ...
                    {
                        if ((recorded_qubit.count(q0) > 0 && recorded_qubit[q0]) || (recorded_qubit.count(q0) > 0 && !gates[j].IsDiagonal()))
                        {
                            add_gate = true;
                    ...
                    if (gate_qubits.size() == 2)
                    {
                        ...
                        if (IsCrossingGate(j))
                        {
                            if (xCZ_collected == xCZ_to_collect)
                                break;
                        ...
                        else if ((recorded_qubit.count(q1) > 0 && recorded_qubit[q1]) || (recorded_qubit.count(q1) > 0 && !gates[j].IsDiagonal()))
                        {
                            add_gate = true;
                    ...
                    if (xCZ_to_collect == xCZ_collected && boundary_qubits_obstructed >= xCZ_collected && current_mode != Config::SimMode::Branch)
                    {
                        ++j;
                        goto exit_inner_loop;
                    }
                    // TODO : Remove and test.
                    else if (xCZ_to_collect < num_xCZ_in_cluster && xCZ_to_collect == xCZ_collected && boundary_qubits_obstructed >= xCZ_collected)
                    {
                        ++j;
                        goto exit_inner_loop;
                    }
        ...
        if (gates[i].GetType() == Gate::Type::cz)
        {
            ...
            for (; j < gates.size() && gates[j].GetType() == Gate::Type::cz; ++j)
            {
                ...
                if (nearest_neigbors)
                {
                    ...
                    if (!((x0 == x1 && (y0 + 1 == y1 || y0 - 1 == y1)) || ((x0 + 1 == x1 || x0 - 1 == x1) && (y0 == y1))))
                    {
            ...
            sort(gates.begin() + i, gates.begin() + i + count_xCZ,
                 [](Gate &g1, Gate &g2)
                 { return g1.GetQubits().front() < g2.GetQubits().front(); });
            sort(gates.begin() + i, gates.begin() + i + count_xCZ,
                 [](Gate &g1, Gate &g2)
                 {
            ...
            if (transition_cycle && count_xCZ)
            {
                ...
                for (idx_size k = j; k < gates.size() && gates[k].GetType() != Gate::Type::cz; ++k)
                    ++next_CZ;
                ...
                for (; i < gates.size(); ++i)
                {
                    ...
                    if (gates[i].GetType() == Gate::Type::x_1_2 || gates[i].GetType() == Gate::Type::y_1_2 || gates[i].GetType() == Gate::Type::h)
                    {
                        ...
                        for (idx_size k = 0; k < xCZ_counter && gates[next_CZ - XYH_count - k].GetType() != Gate::Type::cz; ++k)
...
{
    qp = make_shared<QubitPartition>(config->sim_type == Config::SimType::LosslessH
