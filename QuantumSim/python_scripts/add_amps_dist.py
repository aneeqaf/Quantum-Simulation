...
def ProcessorWrapper(process_id, queue_in, queue_out, binary_vectors_only, outdir, add_partial_res_amps):
  try:
...
def InitializeProcesses(binary_vectors_only, outdir, add_partial_res_amps):
  queue_in = Queue()
  queue_out = Queue()
  for counter in range(multiprocessing.cpu_count()):
    process = Process(target=ProcessorWrapper,
                      args=(counter, queue_in, queue_out, binary_vectors_only, outdir, add_partial_res_amps))
    process.start()
...
def main(cir_dir, num_idx, binary_vectors_only, not_final_amps, add_partial_res_amps):
  ...
  queue_in, queue_out = InitializeProcesses(binary_vectors_only, outdir, add_partial_res_amps)
