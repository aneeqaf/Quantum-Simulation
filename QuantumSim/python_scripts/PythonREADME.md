...
To overcome version incompatibility issues please make sure you specify the arguments after the options and all arguments must appear in the correct order as listed in the documentation. 
...
Logs, amplitude files, and scripts are output in the directory `<circuit_filename>_<depth>_<prefix_bits>_<num_threads>` or `<circuit_filename>_<depth>_<prefix_bits>_<num_threads>_approx_<eps>`  within the directory `log\`,  `amp_vectors\`,  or  `bin\` respectively.
