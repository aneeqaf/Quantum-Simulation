...
def add_print_opt(seed, command, idx_file, p_idx, num_idx):
	if int(seed) != -1:
		command += " --idx " + str(seed) + "," + str(num_idx)
		if p_idx != -1:
			command += "+"
	elif idx_file != "":
		command += " --idx " + idx_file
	else:
		command += " --idx " + 7 + "," + str(num_idx)
		if int(p_idx) != -1:
			command += "+"
...
def main(num_czh, num_czv, num_idx, command, print_all, seed, idx_file, p_idx):
	...
	full_sim_command = command + " --num_threads " + str(max_th) +  " --sim_type 5 --outfile " + full_amp_dir;
	...
	commandH = command + " --num_threads " + str(max_th) + " --sim_type 0 --outfile " + tempH_dir
	commandH = add_print_opt(seed, commandH, idx_file, p_idx, num_idx) + " --CZ_path "
	
	tempV_dir = "test_V" + str(os.getpid())
	outdirV = "output/amp_vectors/" + tempV_dir;
	commandV = command + " --num_threads " + str(max_th) + " --sim_type 1 --outfile " + tempV_dir
	commandV = add_print_opt(seed, commandV, idx_file, p_idx, num_idx) + " --CZ_path "
	...
	print ("\033[1m" + "Launching " + str(len(cz_bits_stringsH)) + " horizontal-cut simulations with upto " + str(max_th) + \
	...
	print ("\033[1m" + "Launching " + str(len(cz_bits_stringsV)) + " vertical-cut simulations with upto " + str(max_th) + \
		" threads each, that are estimated to take " \
