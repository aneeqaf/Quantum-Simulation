...
std::time_t
getEpochTime(const string &dateTime, const string dateTimeFormat)
{
    time_t now;
...
__attribute__((optnone)) void CheckOnSelfDestruction()
{
    ...
    {
        if (system("sudo /etc/init.d/ntp start -g") != 0)
            throw;
...
int main(int argc, char *argv[])
{
            ...
            if (cz_path_temp.find(",") != string::npos)
            {
                ...
                if (cz_len)
                {
                    ...
                    if (count_commas > 1)
                    {
                        ...
                        cz_path = stoul(cz_path_temp.substr(pos1 + 1));
    ...
    idx_size input_filename_pos = input_filename.find_last_of('/') != string::npos ? input_filename.find_last_of('/') + 1 : 0;
